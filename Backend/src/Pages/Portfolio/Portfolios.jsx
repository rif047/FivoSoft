...
export default function Portfolio() {
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            await axios.delete(`${process.env.REACT_APP_API_URL}/api/portfolios/destroy/${id}`);
            fetchData();
                            ...
                            return (
                                <tr key={portfolio._id} className="bg-white border-b hover:bg-gray-100">
                                    ...
                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 flex justify-center">
                                        <img src={`${process.env.REACT_APP_API_URL}/Images/Portfolio/${portfolio.image}`} alt={portfolio.name} className="w-[70px] h-[50px] rounded-md shadow-lg" />
                                    </th>
