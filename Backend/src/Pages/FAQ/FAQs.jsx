...
export default function Faq() {
    ...
    const fetchFaq = async () => {
        const response = await axios.get('http://fivosoft.mepfbd.com/api/faqs');
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            await axios.delete(`http://fivosoft.mepfbd.com/api/faqs/delete/${id}`);
            fetchFaq();
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end">
                <NavLink to={'/faqs/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 py-1 rounded-md mb-2">
                    NEW
                    <AddOutlinedIcon />
                </NavLink>
            </div>
            <table className="w-full text-sm text-left text-gray-500 shadow-md sm:rounded-lg">
                ...
                <tbody>

                    {
                        faqs?.map((faq) => {
                            return (
                                <tr key={faq.que} className="bg-white border-b hover:bg-gray-100">
                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900">
                                        {capitalizeFirst(faq.que)}
                                    </th>
                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900">
                                        {capitalizeFirst(faq.ans)}
                                    </th>
                                    <td className="px-4 py-3 flex justify-center">
                                        <NavLink
                                            to={`/faqs/update/${faq.que}`}
                                            className="mr-3 [&>svg]:text-yellow-400"
                                        >
                                            <EditOutlinedIcon />
                                        </NavLink>
                                        <button onClick={() => handleDelete(faq._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                                    </td>
                                </tr>
                            )
                        })
